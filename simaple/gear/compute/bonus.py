...
class SDIL:
    ...
    def has_negative(self) -> bool:
        return self.value[0] < 0 or self.value[1] < 0 or self.value[2] < 0 or self.value[3] < 0
...
class SDILTableBuilder(pydantic.BaseModel):
    def build(self, gear: Gear):
        bf = BonusFactory()
        cache = {}
        grade_range = [3, 4, 5, 6, 7] if gear.meta.boss_reward else [1, 2, 3, 4, 5, 6, 7]
...
class StatBonusCalculator(pydantic.BaseModel):
    ...
    sdil_table_builder: SDILTableBuilder = pydantic.Field(default_factory=SDILTableBuilder)
    bonus_type_table: CachedBonusTypeTable = pydantic.Field(default_factory=CachedBonusTypeTable)

    _sdil_table: dict[BonusType, list[SDIL]] = pydantic.PrivateAttr(default_factory=dict)
    ...
    def compute(self, stat: Stat, gear: Gear, bonus_count_left: int) -> list[Bonus]:
        ...
        self._grades = [5, 4, 6, 3, 7] if gear.meta.boss_reward else [5, 4, 6, 3, 2, 1, 7]
    ...
    def _search_bonus(self, target_sdil: SDIL, gear: Gear, left: int) -> Optional[list[Bonus]]:
        _dual_bonus_types = {
        ...
        for single_stat_grade, dual_stat_grades in target_sdil.decompose_into_grades(
            ...
            if dual_stat_grades is None:
                result = self._search_bonus_recursive(remaining_sdil, left_count, forbidden_types)
                if result is not None:
                    return result + [self.bonus_factory.create(max_type, single_stat_grade)]
            else:
                for dual_stat_types in combinations(_dual_bonus_types[max_type], len(dual_stat_grades)):
    ...
    def _search_bonus_recursive(self, remaining_sdil: SDIL, left: int, forbidden_bonus_types: list) -> Optional[list[Bonus]]:
        ...
        for bonus_type in self.bonus_type_table.get_types(remaining_sdil):
            ...
            for grade in self._grades:
                ...
                bonus_candidate = self._search_bonus_recursive(decreased_sdil, left - 1, appended_forbidden_bonus_types)
                if bonus_candidate is not None:
                    return bonus_candidate + [self.bonus_factory.create(bonus_type, grade)]
...
class BonusCalculator(GearImprovementCalculator):
    bonus_factory: BonusFactory = pydantic.Field(default_factory=BonusFactory)
    stat_bonus_calculator: StatBonusCalculator = pydantic.Field(default_factory=StatBonusCalculator)
        ...
        for single_property in single_properties:
            ...
            if stat.get(stat_type) > 0:
                ...
                for grade in grades:
                    ...
                    if bonus.calculate_improvement(gear.meta).get(stat_type) == stat.get(stat_type):
