...
STAT_PROP_TYPES = [
    StatProps.INT, StatProps.DEX, StatProps.LUK, StatProps.STR, StatProps.MHP
]
...
class SpellTrace(GearImprovement):
    ...
    def get_weapon_improvement(self, gear: Gear) -> Tuple[int, int, int]:
        attack_basis, stat_basis = _WEAPON_IMPROVEMENTS_ATT_STAT[self.probability][self.get_spell_trace_rank(gear)]
    ...
    def get_glove_improvement(self, gear: Gear) -> Tuple[int, int, int]:
        attack_basis = _GLOBE_IMPROVEMENTS_ATT[self.probability][self.get_spell_trace_rank(gear)]
        stat_basis = 1 if attack_basis == 0 else 0

        return attack_basis, stat_basis, 0

    def get_armor_improvement(self, gear: Gear) -> Tuple[int, int, int]:
        stat_basis, additional_mhp, _ = _ARMOR_IMPROVEMENTS_STAT_MHP_PDD[self.probability][self.get_spell_trace_rank(gear)]  # third value is PDD
        if self.stat_prop_type is StatProps.MHP:
    ...
    def get_accesory_improvement(self, gear: Gear) -> Tuple[int, int, int]:
        stat_basis = _ACCESORY_IMPROVEMENTS_STAT[self.probability][self.get_spell_trace_rank(gear)]
        if self.stat_prop_type is StatProps.MHP:
            stat_basis = stat_basis * 50
    ...
    def get_machine_heart_improvement(self, gear: Gear) -> Tuple[int, int, int]:
        attack_basis = _MACHINE_HEART_IMPROVEMENTS_STAT[self.probability][self.get_spell_trace_rank(gear)]
    ...
    def calculate_improvement(self, gear: Gear) -> Stat:
        ...
        attack_type = StatProps.magic_attack if self.stat_prop_type == StatProps.INT else StatProps.attack_power
        ...
        if gear.is_weapon() or gear.type == GearType.katara:
            ...
            attack_basis, stat_basis, additional_mhp = self.get_machine_heart_improvement(gear)

        improvement += Stat.parse_obj({
            attack_type.value: attack_basis,
            self.stat_prop_type.value: stat_basis
        })

        improvement += Stat(MHP=additional_mhp)

        if (gear.is_armor() or gear.is_accessory()) and self.order == 4 and gear.type != GearType.glove:
            improvement += Stat(
                magic_attack=int(gear.req_job == 0 or math.floor(gear.req_job / 2) % 2 == 1),
                attack_power=int(gear.req_job == 0 or math.floor(gear.req_job / 2) % 2 == 0)
            )
