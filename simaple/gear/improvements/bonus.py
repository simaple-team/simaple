...
class Bonus(GearImprovement):
    ...
    def calculate_improvement(self, gear: Gear) -> Stat:
        ...
        if self.bonus_type in (
            BonusType.MHP, BonusType.MMP
        ):
            return Stat.parse_obj({self.bonus_type.value: gear.req_level // 10 * 30 * grade})

        if self.bonus_type in (
            BonusType.attack_power, BonusType.magic_attack
        ):
            if gear.is_weapon():

                grade_multiplier = ([0, 0, 1, 1.4666, 2.0166, 2.663, 3.4166]
                        if gear.boss_reward else
                        [1, 2.222, 3.63, 5.325, 7.32, 8.777, 10.25])

                basis = gear.stat.attack_power if gear.stat.attack_power > gear.stat.magic_attack else gear.stat.magic_attack

                if gear.type == GearType.sword_zb or gear.type == GearType.sword_zl:
                    if gear.type == GearType.sword_zl:
                        if basis == 100: basis = 102
                        elif basis == 103: basis = 105
                        elif basis == 105: basis = 107
                        elif basis == 112: basis = 114
                        elif basis == 117: basis = 121
                        elif basis == 135: basis = 139
                        elif basis == 169: basis = 173
                        elif basis == 203: basis = 207
                        elif basis == 293: basis = 297
                        elif basis == 337: basis = 342
                        else:
                            print('Not implemented weapon:\n' + str(self.gear))
                    level_multiplier = 6 if gear.req_level > 180 else (5 if gear.req_level > 160 else (4 if gear.req_level > 110 else 3))
                else:
                    if gear.boss_reward:
                        level_multiplier = 18 if gear.req_level > 160 else (15 if gear.req_level > 150 else (12 if gear.req_level > 110 else 9))
                    else:
