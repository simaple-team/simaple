...
class DamageLogic(BaseModel, metaclass=TaggedNamespacedABCMeta(kind="DamageLogic")):
    ...
    def _get_general_damage_factor(self, stat: Stat) -> float:
        return (1 + (stat.boss_damage_multiplier + stat.damage_multiplier) * 0.01) * (1 + 0.01 * stat.final_damage_multiplier)
    ...
    def get_dot_factor(self, stat: Stat, armor: int = 300) -> float:
        return self.get_base_stat_factor(stat) * self.get_attack_type_factor(stat) * self.attack_range_constant * 0.01
...
class STRBasedDamageLogic(DamageLogic):
    ...
    def get_base_stat_factor(self, stat: Stat) -> float:
        return stat.get_base_stat_coefficient(BaseStatType.STR) * 4 + stat.get_base_stat_coefficient(BaseStatType.DEX)
...
class DEXBasedDamageLogic(DamageLogic):
    ...
    def get_base_stat_factor(self, stat: Stat) -> float:
        return stat.get_base_stat_coefficient(BaseStatType.DEX) * 4 + stat.get_base_stat_coefficient(BaseStatType.STR)
...
class LUKBasedDamageLogic(DamageLogic):
    ...
    def get_base_stat_factor(self, stat: Stat) -> float:
        return stat.get_base_stat_coefficient(BaseStatType.LUK) * 4 + stat.get_base_stat_coefficient(BaseStatType.DEX)
