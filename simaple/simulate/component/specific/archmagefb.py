...
class PoisonRegionState(TypedDict):
    cooldown: Cooldown
    lasting: Lasting
    consumable: Consumable  # Areas generated by skill
    dynamics: Dynamics
...
class PoisonRegionComponent(
    ...
    @reducer_method
    def use(self, _: None, state: PoisonRegionState):
    ...
    def elapse(self, time: float, state: PoisonRegionState):
        ...
        return state, event + [EmptyEvent.elapsed(time)]

    @reducer_method
    def explode(self, _: None, state: PoisonRegionState):
        consumable = state["consumable"].model_copy()

        current_areas = state["consumable"].stack

        if current_areas == 0:
            return state, []

        consumable.consume_all()
        state["consumable"] = consumable

        return state, [
            EmptyEvent.dealt(self.explode_damage, (current_areas * 0.6 + 0.4) * self.explode_hit),
        ]
