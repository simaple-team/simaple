...
class ContextDispatcher(Dispatcher):
    ...
    def __init__(
        self, origin_name, origin_method, defined_action: Action, context: Dispatcher
    ):
        self._defined_action = defined_action
        self._context = context
        self._signature = message_signature(
            {"name": origin_name, "method": origin_method, "payload": None}
        )
...
def addon_to_dispatcher(
    origin_name: str, addon: _ComponentAddon, context_dispatcher: Dispatcher
):
...
class ReducerMethodWrappingDispatcher(Dispatcher):
    ...
    def get_context_synced_dispatcher(self, context_dispatcher: Dispatcher):
        return TandemDispatcher(
            self,
            [
                addon_to_dispatcher(self._name, addon, context_dispatcher)
...
def listening_action_reducer(
    listening_actions: dict[str, Union[str, "StaticPayloadReducerInfo"]],
    target_reducer,
):
    for listening_signature, reducer_info in listening_actions.items():
