    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls % 1000 == 0:  # 1000 스텝마다 평가
            ...
            if episode_reward > self.best_reward:
                ...
                # 오퍼레이션 저장
                operations = [op for op in self.eval_env.env.engine.operation_logs()]
...
def run_training_sb3(
    ...
    # Gym 환경 생성
    env = SimapleEnv(player, damage_calculator, target_time, max_steps)
    ...
    # 평가용 환경 생성
    eval_env = SimapleEnv(SimaplePlayer(get_engine(environment)), damage_calculator, target_time, max_steps)
    ...
    # 학습된 모델 평가
    print("\n학습된 모델 평가")
    evaluate_model_sb3(model, eval_env)
    
    return model


def evaluate_model_sb3(model, env, num_episodes=5):
    """학습된 SB3 PPO 모델 평가"""
    ...
    # 실행된 액션 시퀀스 저장
    operations = [op for op in env.engine.operation_logs()]
    with open("sb3_executed_actions.txt", "w") as f:
        for op in operations:
            if hasattr(op, 'playlogs') and op.playlogs:
                if hasattr(op, 'command'):
                    try:
