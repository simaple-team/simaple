...
def _get_hyperstat_basis_from_spec() -> dict[StatProps, list[Stat]]:
    ...
    return {StatProps(basis.name): [ex_stat.stat for ex_stat in basis.values] for basis in hyperstat_basis}
...
def get_hyperstat_lists() -> list[tuple[StatProps, list[Stat]]]:
    return list(sorted(_get_hyperstat_basis_from_spec().items(), key=lambda x: x[0].value))
