...
def test_artifact_generator():
    ...
    generator.get_optimal_artifact(12, ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"])
...
def test_artifact():
    artifact = Artifact(
        ...
        effects=[
            ArtifactEffect(
                name=effect_name,
                effects=[ExtendedStat(stat=Stat(INT=idx * effect_idx)) for idx in range(11)],
            )
            for effect_idx, effect_name in enumerate("ABCDEFGHI")
        ],
    )

    assert artifact.get_extended_stat() == ExtendedStat(stat=Stat(INT=1 * (0 + 1 + 2) + 2 * (3 + 4 + 5)))
