...
from simaple.request.adapter.item import get_gears, get_symbols
...
def test_item_equipment(character_item_equipment_response: CharacterItemEquipment):
    ...
    assert gears[0].stat == Stat.model_validate(
        {
            "STR": 5.0,
            "LUK": 178.0,
            "INT": 226.0,
    ...
    assert gears[0].potential.get_stat() == Stat(INT_multiplier=18)
    assert gears[0].potential.get_action_stat() == ActionStat(cooltime_reduce=2)
