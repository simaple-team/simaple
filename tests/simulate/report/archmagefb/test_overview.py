...
def test_buff_view(archmagefb_engine: MonotonicEngine):
    current_buff_stat = archmagefb_engine.get_viewer()("buff")

    archmagefb_engine.play(dict(name="에픽 어드벤쳐", method="use", payload=None))

    assert archmagefb_engine.get_viewer()("buff") == current_buff_stat + Stat(
        damage_multiplier=10
    )


def test_poison_nova(archmagefb_engine: MonotonicEngine):
    actions: list[Action] = [
    ...
    for action in actions:
        events += archmagefb_engine.play(action)

    assert "포이즌 노바.trigger" in [message_signature(e) for e in events]


def test_poison_chain(archmagefb_engine: MonotonicEngine):
    actions: list[Action] = [
