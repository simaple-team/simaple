...
def compute_total_damage_coefficient(events: list[Event]) -> float:
    return sum(
        [cast(float, e["payload"]["damage"] * e["payload"]["hit"]) for e in events if e["tag"] == Tag.DAMAGE and e["payload"] is not None]
    )


def total_delay(events: list[Event]) -> int:
    return sum([e["payload"]["time"] for e in events if e["tag"] == Tag.DELAY and e["payload"] is not None])
...
def pipe(initial_state: S, *args: Callable[[S], tuple[S, Union[Event, list[Event]]]]) -> tuple[S, list[Event]]:
    state = initial_state
