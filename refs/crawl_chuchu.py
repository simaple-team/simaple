...
from simaple.core.jobtype import JobType, _kms_job_names

CLASS_NAMES: list[tuple[str, JobType]] = [
    v for v in list(_kms_job_names.items()) if v[1] != JobType.virtual_maplestory_m
]


def crawl_chuchu_ranking(job_name: str) -> List[Dict[str, Any]]:
    base_url = "https://chuchu.gg/api/getRanking/overall"
    params = {"class": job_name, "page": 1, "world_name": "이노시스"}
...
def save_characters(characters: List[Dict[str, Any]], file_name: str):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        yaml.dump(
            {"characters": characters}, f, allow_unicode=True, sort_keys=False, indent=2
        )
